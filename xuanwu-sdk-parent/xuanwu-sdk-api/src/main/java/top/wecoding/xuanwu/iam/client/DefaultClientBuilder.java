...
public class DefaultClientBuilder implements ClientBuilder {
  ...
  @Getter private final ClientConfiguration clientConfig = new ClientConfiguration();
  ...
  DefaultClientBuilder(ResourceFactory resourceFactory) {
    ...
    if (Strings.hasText(props.get(DEFAULT_CLIENT_CACHE_TTL_PROPERTY_NAME))) {
      clientConfig.setCacheManagerTtl(
          Long.parseLong(props.get(DEFAULT_CLIENT_CACHE_TTL_PROPERTY_NAME)));
    }

    if (Strings.hasText(props.get(DEFAULT_CLIENT_CACHE_TTI_PROPERTY_NAME))) {
      clientConfig.setCacheManagerTti(
          Long.parseLong(props.get(DEFAULT_CLIENT_CACHE_TTI_PROPERTY_NAME)));
    }
    ...
    if (Strings.hasText(props.get(DEFAULT_CLIENT_API_BASE_PROPERTY_NAME))) {
      ...
      // remove backslashes that can end up in file when it's written programmatically, e.g. in a
      // test
    ...
    if (Strings.hasText(props.get(DEFAULT_CLIENT_CONNECTION_TIMEOUT_PROPERTY_NAME))) {
      clientConfig.setConnectionTimeout(
          Integer.parseInt(props.get(DEFAULT_CLIENT_CONNECTION_TIMEOUT_PROPERTY_NAME)));
    }

    if (Strings.hasText(props.get(DEFAULT_CLIENT_PROXY_PORT_PROPERTY_NAME))) {
      clientConfig.setProxyPort(
          Integer.parseInt(props.get(DEFAULT_CLIENT_PROXY_PORT_PROPERTY_NAME)));
    }
    ...
    if (Strings.hasText(props.get(DEFAULT_CLIENT_REQUEST_TIMEOUT_PROPERTY_NAME))) {
      clientConfig.setRetryMaxElapsed(
          Integer.parseInt(props.get(DEFAULT_CLIENT_REQUEST_TIMEOUT_PROPERTY_NAME)));
    }

    if (Strings.hasText(props.get(DEFAULT_CLIENT_RETRY_MAX_ATTEMPTS_PROPERTY_NAME))) {
      clientConfig.setRetryMaxAttempts(
          Integer.parseInt(props.get(DEFAULT_CLIENT_RETRY_MAX_ATTEMPTS_PROPERTY_NAME)));
    }
    // @formatter:on
  }
  ...
  public ApiClient build() {
    ...
    if (StringUtils.isNotBlank(clientConfig.getProxyHost())) {
      ...
      if (!StringUtils.isAnyBlank(username, password)) {
        basicAuthenticationInterceptor =
            Optional.of(new BasicAuthenticationInterceptor(username, password));
      }
    }
    ...
    ClientCredentialsProvider clientCredentialsProvider =
        this.clientConfig.getClientCredentialsProvider();
    if (clientCredentialsProvider == null && this.clientCredentials != null) {
      this.clientConfig.setClientCredentialsProvider(
          new DefaultClientCredentialsProvider(this.clientCredentials));
    } else if (clientCredentialsProvider == null) {
      this.clientConfig.setClientCredentialsProvider(
          new DefaultClientCredentialsProvider(this.clientConfig));
    }

    Object apiToken =
        this.clientConfig.getClientCredentialsProvider().resolveCredentials().getCredentials();
   ...
   *
   * @param clientConfig the current clientConfig
   * @return a {@link PoolingHttpClientConnectionManagerBuilder} initialized with default
   *     configuration
   */
