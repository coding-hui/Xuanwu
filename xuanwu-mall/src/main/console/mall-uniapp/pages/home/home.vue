	...
	<view class="container">
		...
		<view class="content">
			...
			<view class="section-1">
				...
				<navigator class="item" open-type="navigate" url="###" hover-class="none">
			...
			<view class="section-2">
				<button v-for="(table, index) in tables" :key="index" class="item" @click="toCreateOrder(table)">
					{{table.code}}
				</button>
			</view>
			<!-- section-2 end -->
...
<script>
	...
	export default {
		...
		async onLoad() {
			let tables = await listOrderTables({})
			this.tables = tables.records
		},
		methods: {
			toCreateOrder(table) {
				let orderId = table && table.order ? table.order.id : null
				uni.navigateTo({
					url: `/pages/index/index?orderTableId=${table.id}`
				});
			}
		}
	}
