	...
	<view class="container">
		...
		<swiper :current="tabIndex" :duration="300" class="swiper" :show-scrollbar="false">
			...
			<swiper-item @touchmove.stop="handleSwiperItemChange">
				<view v-if="orders.length == 0" class="no-order-content">
					<image src="https://go.cdn.heytea.com/storage/ad/2020/05/20/0bdb360866d94aa4a4404c0b676a1982.jpg">
					</image>
					<view class="tips">
						<view>您今天还没有下单</view>
					</view>
					<button @tap="toHome" type="primary" class="font-size-lg" hover-class="none">去下单</button>
				</view>
					...
					<view class="wrapper">
						<view class="order-list">
							<navigator class="order" v-for="(order, index) in orders" :key="index" open-type="navigate"
								...
								<view class="header">
									<view class="flex-fill font-size-medium">三味鱼火锅</view>
									<view class="status">
										<view>{{orderStatusTexts[order.status]}}</view>
										<image src="/static/images/common/black_arrow_right.png"></image>
									</view>
								</view>
								<scroll-view scroll-x>
									<view class="images">
										<image :src="item.image" v-for="(item, index) in order.items" :key="index">
										</image>
									</view>
								</scroll-view>
								<view class="info">
									<view class="left">
										<view>订单桌号：{{ order.tableCode }}</view>
