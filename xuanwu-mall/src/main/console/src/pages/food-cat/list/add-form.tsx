...
import {
  createFoodCat,
  CreateFoodCatRequest,
  getFoodCat,
  updateFoodCat,
  UpdateFoodCatRequest
} from '@/api/food/food-cat';
...
function AddFoodCat(props: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      let res;
      if (isCreated) {
        res = await createFoodCat(values as CreateFoodCatRequest);
      } else {
        res = await updateFoodCat(foodCatId, values as UpdateFoodCatRequest);
      }
      if (res.success) {
