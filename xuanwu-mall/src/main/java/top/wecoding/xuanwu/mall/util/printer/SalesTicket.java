...
public class SalesTicket implements Printable {
	...
	public int print(Graphics graphics, PageFormat pageFormat, int pageIndex) throws PrinterException {
		...
		// 虚线设置
		g2.setStroke(new BasicStroke(0.5f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 4.0f, new float[] { 4.0f }, 0.0f));
		...
		line += heigth;
		// 设置开台时间
		g2.drawString("下单时间:" + order.getCreatedAt().format(timeFormatter), (float) x + 5, (float) y + line);
		line += heigth;
		...
		if (!CollectionUtils.isEmpty(orderItems)) {
			for (OrderItem orderItem : orderItems) {
				...
				g2.drawString(orderItem.getFoodQuantity().toString(), (float) maxWidth - 100, (float) y + line);
				g2.drawString(orderItem.getFoodPrice().toString(), (float) maxWidth - 60, (float) y + line);
				g2.drawString(orderItem.getFoodPrice().multiply(BigDecimal.valueOf(orderItem.getFoodQuantity())).toString(), (float) maxWidth - 30, (float) y + line);
				line += heigth + 3;
			}
		}
		g2.drawLine((int) x, (int) (y + line), (int) x + 227, (int) (y + line));
		...
		line += heigth;
		g2.drawString("打印时间:" + LocalDateTime.now().format(timeFormatter), (float) x + 5, (float) y + line);
