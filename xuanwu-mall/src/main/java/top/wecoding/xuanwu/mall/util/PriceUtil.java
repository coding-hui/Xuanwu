...
public class PriceUtil {
	...
	public static BigDecimal calcPayAmount(Order order) {
		...
		return selfGetAmount(order.getTotalAmount()).add(selfGetAmount(order.getFreightAmount()))
		...
		for (OrderItem orderItem : orderItems) {
			if (orderItem.getPromotionAmount() != null) {
				promotionAmount = promotionAmount
					.add(orderItem.getPromotionAmount().multiply(new BigDecimal(orderItem.getFoodQuantity())));
			}
		}
