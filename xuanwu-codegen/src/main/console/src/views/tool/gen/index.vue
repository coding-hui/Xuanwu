...
<!--      </el-col>-->
    ...
    <el-table v-loading="loading" :data="tableList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="版本" align="center" prop="version" width="60">
        <template #default="scope">
          <span style="font-weight: 600">{{ scope.row.version }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="preview.title" v-model="preview.open" width="80%" top="5vh" append-to-body class="scrollbar">
      <el-tabs v-model="preview.activeName">
        ...
        >
          ...
          <pre><code ref="codeBlock" class="language-java">{{ value }}</code></pre>
        </el-tab-pane>
      </el-tabs>
    </el-dialog>
...
function handlePreview(row) {
  previewTable(row.id).then(response => {
    ...
    preview.value.activeName = "entity.java";
  });
}
/** 复制代码成功 */
...
function handleEditTable(row) {
  ...
  router.push({ path: "/generator/edit/index/" + id, query: { pageNum: queryParams.value.pageNum } });
}
/** 删除按钮操作 */
