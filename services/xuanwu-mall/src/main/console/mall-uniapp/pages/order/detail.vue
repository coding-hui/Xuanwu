	...
	<view class="container">
		...
		<view class="content" v-if="Object.keys(order).length>0">
			<view class="order-box">
				...
				<view class="text-color-primary font-size-lg font-weight-bold mb-10">{{orderStatusTexts[order.status]}}
				</view>
				...
				<view class="order-info">
					<list-cell padding="50rpx 40rpx" line-right bgcolor="#f6f6f6">
						<view class="w-100 d-flex align-items-center">
							<view class="flex-fill d-flex flex-column overflow-hidden">
								<view class="font-size-lg mb-10">三味鱼火锅</view>
					...
					<list-cell padding="0 40rpx" line-right bgcolor="#f6f6f6">
						<view class="w-100 d-flex flex-column">
							<view class="d-flex align-items-center mt-40" v-for="(item, index) in order.orderItems"
								:key="index">
								<view class="flex-fill d-flex flex-column">
									<view class="font-size-base mb-10">{{ item.foodName }}</view>
									<!-- <view class="font-size-extra-sm text-color-assist">
										{{ materialsText(item.materials) }}
									</view> -->
								</view>
								<view class="flex-shrink-0 font-weight-bold ml-40">x{{ item.foodQuantity }}</view>
								<view class="flex-shrink-0 font-weight-bold ml-40">￥{{ item.foodPrice }}</view>
							</view>
					...
					<list-cell padding="30rpx 40rpx" last bgcolor="#f6f6f6" style="border-radius: 0 0 30rpx 30rpx;">
						<view class="w-100 d-flex flex-column font-size-extra-sm text-color-assist">
							<view class="mb-10">下单时间：{{ order.createdAt }}</view>
							<view class="mb-10">订单桌号：{{ order.tableCode }}</view>
							<view class="mb-10">订单编号：{{ order.orderSn }}</view>
							<!-- <view class="mb-10">备注信息：{{ order.remarks }}</view> -->
						</view>
					</list-cell>
				</view>
				<view v-if="order.status != 1" @tap="pay" class="order-invoice font-size-base d-flex justify-content-between align-items-center">
					<view>合计￥{{ order.totalAmount }}</view>
...
<script>
	...
	export default {
		...
		async onLoad(options) {
			...
			console.log(id, options, 'ooo')
			if (id) {
				this.order = await getOrderDetails(id)
			}
