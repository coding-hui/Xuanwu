<template>
	<view class="container">
		...
		<view class="bg-white pt-30 mb-20">
			<view class="font-size-medium font-weight-bold mb-30 pl-30">菜品列表</view>
				...
				<view class="w-100 d-flex align-items-center overflow-hidden">
					<scroll-view class="flex-fill overflow-hidden" scroll-x>
						<view class="w-100 d-flex align-items-center">
							...
							<text class="pro-img" v-for="(item, index) in cart" :key="index">{{item.foodName}}</text>
						</view>
					</scroll-view>
		...
		<view class="footer">
			...
			<button type="primary" @tap="pay">结账</button>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			pay() {
				createOrder({ payType: 0, orderId: orderId, tableCode: orderTableCode })
			}
		},
		computed: {
			cartNum() {
				return this.cart.reduce((acc, cur) => acc + cur.foodQuantity, 0)
			},
			cartAmount() {
				return this.cart.reduce((acc, cur) => acc + cur.foodQuantity * cur.foodPrice, 0)
			},
			remark() {
