...
public class PrinterServiceImpl extends BaseServiceImpl<Printer, Long> implements PrinterService {
  ...
  @Override
  public PageResult<Printer> listPrinterService(
      PrinterServicePageRequest query, Pageable pageable) {
    Page<Printer> pageResult =
  ...
  public void printSalesTicket(OrderDetail orderDetail, Integer printType) {
    ...
    for (Printer printerService : printerServices) {
      ...
      try {
        ...
        log.error(
            "Failed to print sales ticket, order: {}, printer: {}",
            orderDetail.getOrder().getId(),
            printerService.getName(),
            e);
      }
    }
  }
  ...
  public boolean printTestPage(Long id) {
    ...
    String orderDetailJsonStr =
        printerConfig.getDemoOrder().getContentAsString(StandardCharsets.UTF_8);
  ...
  private boolean printEscPosPage(Printer printer, OrderDetail orderDetail) {
    try {
      ...
      String kitchenTpl =
          printerConfig.getKitchenTemplate().getContentAsString(StandardCharsets.UTF_8);
