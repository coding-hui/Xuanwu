...
public abstract class AbstractTemplateService implements TemplateService {
  ...
  protected HashSet<String> getImportList(TableEntity genTable) {
    ...
    for (ColumnEntity column : columns) {
      if (!column.isSuperColumn() && GenConstants.TYPE_DATE.equals(column.getGenType())) {
        importList.add("java.util.Date");
      } else if (!column.isSuperColumn()
          && GenConstants.TYPE_BIGDECIMAL.equals(column.getGenType())) {
  ...
  protected String getGenPath(
      TableEntity table, CodeGenConfig.Template template, final VelocityContext context) {
    String genPath;
    ...
    if (isFrontendTemplate(template.getName())) {
      ...
      genPath =
          table.getBackendPath() + PROJECT_PATH + "/" + StringUtils.replace(packageName, ".", "/");
    }
