...
public class Classes {
  ...
  private static final ClassLoaderAccessor THREAD_CL_ACCESSOR =
      new ExceptionIgnoringAccessor() {
        protected ClassLoader doGetClassLoader() {
          return Thread.currentThread().getContextClassLoader();
        }
      };

  private static final ClassLoaderAccessor CLASS_CL_ACCESSOR =
      new ExceptionIgnoringAccessor() {
        protected ClassLoader doGetClassLoader() {
  ...
  public static Class findClassByName(String className) {
    try {
      ...
      throw new IllegalArgumentException(
          String.format("Unable to load class named [%s]", className));
    }
  }

  /**
   * Determines if the class or interface represented by this object is either the same as, or is a
   * superclass or superinterface of, the class or interface represented by the specified parameter.
   *
   ...
   * @return the value indicating whether objects of the type can be assigned to objects of this
   *     class.
   */
  ...
  public static <T> Class<T> forName(String fqcn) throws ClassNotFoundException {
    ...
    if (clazz == null) {
      String msg =
          "Unable to load class named ["
              + fqcn
              + "] from the thread context, current, or system/application ClassLoaders.  All"
  ...
  public static <T> T newInstance(Class<T> clazz) {
    ...
    try {
      ...
      throw new InstantiationException(
          String.format("Unable to instantiate class [%s]", clazz.getName()), e);
    }
  }
  ...
  public static <T> T instantiate(Constructor<T> ctor, Object... args) {
    try {
      ...
      throw new InstantiationException(
          String.format("Unable to instantiate instance with constructor [%s]", ctor), e);
    }
  }
