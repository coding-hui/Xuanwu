package top.wecoding.xuanwu.cache.redis;
...
import top.wecoding.xuanwu.cache.base.CacheHashKey;
import top.wecoding.xuanwu.cache.base.CacheKey;
...
public class RedisHelper {
  ...
  public byte[] keySerialize(String redisKey) {
    Assert.notNull(redisKey, "redis key cannot be null");
    return RedisSerializer.string().serialize(redisKey);
  }

  public String keyDeserialize(byte[] redisKey) {
    Assert.notNull(redisKey, "redis key cannot be null");
    return RedisSerializer.string().deserialize(redisKey);
  }
  ...
  public <T> void set(String key, T value, Function<T, byte[]> mapper) {
    redisTemplate.execute(
        (RedisCallback<Object>) redis -> redis.set(keySerialize(key), mapper.apply(value)));
  }
  ...
  /**
   * 存放 key value 对到 redis 如果 key 已经存在， SETNX 命令将返回 false， 即不执行任何操作，
   *
   * @param key 缓存key
   * @param value 缓存value
  ...
  /** 返回 key 所关联的 value 值，采用 jdk 序列化 如果 key 不存在那么返回特殊值 nil 。 */
  @Nullable
  public <T> T getByJdkSer(String key) {
    return get(key, (value) -> (T) RedisSerializer.java().deserialize(value));
  }

  /** 返回 key 所关联的 value 值，采用 json 序列化 如果 key 不存在那么返回特殊值 nil 。 */
  @Nullable
  ...
  public <T> T getByJsonSer(String key, Class<T> clazz, Supplier<T> loader) {
    T value = getByJsonSer(key, clazz);
    if (value != null) {
      return value;
    }
    value = loader.get();
   ...
   * @return 扫描结果
   */
  public void sScanBytes(
      String key, String pattern, @Nullable Long count, Consumer<byte[]> consumer) {
    redisTemplate.execute(
        (RedisCallback<Object>)
            redis -> {
              ScanOptions.ScanOptionsBuilder builder = ScanOptions.scanOptions().match(pattern);
   ...
   * 将哈希表 key 中的域 field 的值设为 value 。 如果 key 不存在，一个新的哈希表被创建并进行 HSET 操作。 如果域 field 已经存在于哈希表中，旧值将被覆盖。
   */
  public <F, V> void hSet(CacheHashKey hashKey, V value) {
    safeCheckHashKey(hashKey);
    hashOps.put(hashKey.getKey(), hashKey.getField(), value);
  }
  ...
  /** 返回哈希表 key 中给定域 field 的值。 */
  @Nullable
  public <T> T hGet(CacheHashKey hashKey) {
    safeCheckHashKey(hashKey);
    return (T) hashOps.get(hashKey.getKey(), hashKey.getField());
  }
  ...
  /** 删除哈希表 key 中的一个指定域。 */
  public Long hDel(CacheHashKey hashKey) {
    safeCheckHashKey(hashKey);
    return hashOps.delete(hashKey.getKey(), hashKey.getField());
  }
  ...
  /** 查看哈希表 key 中，给定域 field 是否存在。 */
  public Boolean hExists(CacheHashKey hashKey) {
    safeCheckHashKey(hashKey);
    return hashOps.hasKey(hashKey.getKey(), hashKey.getField());
  }
   ...
   * 位(bit)有符号数字表示之内。
   */
  public Long hIncrBy(CacheHashKey hashKey, long value) {
    safeCheckHashKey(hashKey);
    String key = hashKey.getKey();
    Object field = hashKey.getField();
    return hashOps.increment(key, field, value);
  }
   ...
   * 命令类似，请查看 INCRBYFLOAT 命令获取更多相关信息。
   */
  public Double hIncrByFloat(CacheHashKey hashKey, double value) {
    safeCheckHashKey(hashKey);
    String key = hashKey.getKey();
    Object field = hashKey.getField();
    return hashOps.increment(key, field, value);
  ...
  public Long bitCount(String key, long start, long end) {
    return redisTemplate.execute(
        (RedisCallback<Long>) redis -> redis.bitCount(keySerialize(key), start, end));
  }
  ...
  public List<Long> bitField(String key, BitFieldSubCommands commands) {
    return redisTemplate.execute(
        (RedisCallback<List<Long>>) redis -> redis.bitField(keySerialize(key), commands));
  }
  ...
  public Long bitPos(String key, boolean bit) {
    return redisTemplate.execute(
        (RedisCallback<Long>) redis -> redis.bitPos(keySerialize(key), bit));
  }
  ...
  public Long bitPos(String key, boolean bit, Range<Long> range) {
    return redisTemplate.execute(
        (RedisCallback<Long>) redis -> redis.bitPos(keySerialize(key), bit, range));
  }
  ...
  public Boolean getBit(String key, long offset) {
    return redisTemplate.execute(
        (RedisCallback<Boolean>) redis -> redis.getBit(keySerialize(key), offset));
  }
  ...
  public Boolean setBit(String key, long offset, boolean value) {
    return redisTemplate.execute(
        (RedisCallback<Boolean>) redis -> redis.setBit(keySerialize(key), offset, value));
  }

  private void safeCheckHashKey(CacheHashKey hashKey) {
    Assert.notNull(hashKey, "cache key cannot be null");
    Assert.notNull(hashKey.getKey(), "cache hash key cannot be null");
    Assert.notNull(hashKey.getField(), "cache hash key field cannot be null");
  }
}
