...
import top.wecoding.xuanwu.ai.minimax.api.MiniMaxAiApi.ChatCompletionChunk.ChunkChoice;
...
import top.wecoding.xuanwu.ai.minimax.api.MiniMaxAiApi.ChatCompletionMessage.ChatCompletionFunction;
 ...
 * @since 0.10
 */
public class MiniMaxAiStreamFunctionCallingHelper {
	...
	private ChunkChoice merge(ChunkChoice previous, ChunkChoice current) {
		...
		return new ChunkChoice(finishReason, index, message);
	}
	...
	public ChatCompletion chunkToChatCompletion(ChatCompletionChunk chunk) {
		...
		return new MiniMaxAiApi.ChatCompletion(chunk.id(), choices, chunk.created(), chunk.model(),
